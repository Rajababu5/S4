...
bot = telebot.TeleBot('7022306555:AAFfkqCTjJ_B_E_6aJEHRTH9Fi9JJoriMnU')
# Admin user IDs
admin_id = {"6769245930"}
...
def redeem_key_command(message):
    ...
    if len(command) == 2:
        ...
        if key in keys:
            ...
            del keys[key]
            save_keys()
            response = f"✅𝐊𝐞𝐲 𝐫𝐞𝐝𝐞𝐞𝐦𝐞𝐝 𝐒𝐮𝐜𝐜𝐞𝐬𝐟𝐮𝐥𝐥𝐲!: {users[user_id]}"
        else:
    ...
    if user_id in users:
        ...
        if user_id not in admin_id:
            if user_id in bgmi_cooldown:
                ...
                if time_since_last_attack < COOLDOWN_TIME:
                    ...
                    response = f"❌ 𝘾𝙊𝙊𝙇𝘿𝙊𝙒𝙉 ❌\n\n {cooldown_remaining} 𝐒𝐞𝐜𝐨𝐧𝐝 𝐛𝐚𝐚𝐝.\n\n🔲 𝐒𝟒 𝐎𝐅𝐅𝐈𝐂𝐈𝐀𝐋 🔲"
                    bot.reply_to(message, response)
                    return
        ...
        if len(command) == 4:
            ...
            try:
                ...
                time = int(command[3])
                if time > 300:
                    response = "⚠️𝐄𝐑𝐑𝐎𝐑:300 𝐒𝐄 𝐓𝐇𝐎𝐃𝐀 𝐊𝐀𝐌 𝐓𝐈𝐌𝐄 𝐃𝐀𝐀𝐋 "
                else: 
...
def welcome_start(message):
    ...
    response = f'''𝙒𝙀𝙇𝘾𝙊𝙈𝙀 𝙏𝙊 𝙎4 𝙁𝘼𝙈𝙄𝙇𝙔, {user_name}! 𝐓𝐡𝐢𝐬 𝐢𝐒 𝐘𝐎𝐔𝐑 𝐅𝐀𝐓𝐇𝐑𝐞𝐫𝐒 𝐁𝐨𝐓 𝐒𝐞𝐫𝐯𝐢𝐜𝐞.
🤖𝐔𝐒𝐄 𝐇𝐄𝐋𝐏 𝐂𝐎𝐌𝐌𝐀𝐍𝐃: /help
'''
    bot.reply_to(message, response)
...
def welcome_plan(message):
    user_name = message.from_user.first_name
    response = f'''{user_name}, 𝐏𝐋𝐀𝐍 𝐃𝐄𝐊𝐇𝐄𝐆𝐀 BHAI 🍁:
